...
def convert_images_to_pdf(
    ...
    """
    Convert a folder of images into a single PDF with optional downscaling.

    Args:
        input_dir: Folder containing images
        output_pdf: Path to output PDF (e.g., out/book.pdf)
        scale: Uniform scale factor (e.g., 0.5 halves both width and height)
        max_width: Max width in pixels (fit inside)
        max_height: Max height in pixels (fit inside)
        jpeg_quality: JPEG quality for embedding (higher=better/bigger). Typical 60–90.
        dpi: Set a DPI hint in the PDF metadata (e.g., 150)
    ...
    imgs = [p for p in input_dir.iterdir() if p.is_file() and is_image(p)]
    ...
    for i, p in enumerate(imgs, 1):
        try:
            im = load_image(p)
            im = maybe_resize(im, scale=scale, max_w=max_width, max_h=max_height)
    ...
    print(
        f"Done. Wrote {output_pdf} with {len(processed)} pages. Total pixels: {total_px:,}"
    )
